...
export class Animal {
  constructor(world, rng, x, z, herd) {
    this.world = world;
    this.rng = rng;
    this.herd = herd;
    this.alive = true;
    this.health = CONFIG.nature.animalHealth;
    ...
    const g = new THREE.Group();
    const body = new THREE.Mesh(ANIMAL_BODY, MAT.hide);
    body.position.y = 0.95;
    body.castShadow = true;
    const head = new THREE.Mesh(new THREE.ConeGeometry(0.34, 0.6, 5), MAT.hide);
    head.rotation.z = -Math.PI / 2;
    head.position.set(0.95, 1.05, 0);
    const snout = new THREE.Mesh(ANIMAL_SNOUT, MAT.hideDark);
    snout.rotation.z = -Math.PI / 2;
    snout.position.set(1.3, 1.0, 0);
    const tail = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.5, 4), MAT.hideDark);
    ...
    for (const [lx, lz] of [[0.5, 0.28], [0.5, -0.28], [-0.5, 0.28], [-0.5, -0.28]]) {
      const leg = new THREE.Mesh(GEO.leg, MAT.hideDark);
  ...
  update(dt, nearestAgentDist, nearestAgentPos) {
    ...
    // Subtle gait bob so it reads as alive.
    this.mesh.position.y += Math.abs(Math.sin(performance.now() * 0.006 + this.pos.x)) * 0.05 * (speed > 1 ? 1 : 0.2);
  }
